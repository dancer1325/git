...
* Hooks
  * := programs | hooks directory /
    * | git's execution's CERTAIN point, can trigger actions
    * path
      * by default, `$GIT_DIR/hooks`
      * if you want to change -> [`core.hooksPath`](config/core.md) 
  * if they do NOT have the executable bit set -> are ignored
  * BEFORE Git invokes a hook, 👀it changes its working directory 👀
    * `$GIT_DIR` | bare repository or
      * EXCEPT to, hooks / triggered | push ('pre-receive', 'update', 'post-receive', 'post-update', 'push-to-checkout')
        * == ALWAYS executed | `$GIT_DIR` 
    * `GIT_WORK_TREE` | non-bare repository
  * ways to pass arguments
    * environment,
    * command-line arguments,
    * stdin

* `GIT_DIR`, `GIT_WORK_TREE`, etc., -- are -- exported
  * -> Git commands run by the hook -- can correctly -- locate the repository 
  * if your hook needs to invoke Git commands | foreign repository OR | SAME repository's DIFFERENT working tree -> clear these environment variables
    ```
...
* `git init`
  * copy hooks | NEW repository -- based on -- [configuration](git-init.md#template-directory)
    * "default hooks" == default template -- shipped with -- Git
...
* hook / 
  * -- invoked 
    * by -- linkgit:git-commit[1]
    * -- BEFORE 
      * obtaining the proposed commit log message &
      * making a commit
  * if you want to bypass -> `--no-verify` option
  * NO parameters
  * if you exit the script / non-zero -> `git commit` command aborts commiting
  * default 'pre-commit' hook
    * if it catches introduction of lines / trailing whitespaces -> aborts the commit
    * if `hooks.allownonascii` unset OR `false` -> prevents using non-ASCII filenames
  
* ALL `git commit` hooks -- are invoked with the -- environment variable `GIT_EDITOR=:`
  * use case
    * command does NOT bring up an editor / modify the commit message
